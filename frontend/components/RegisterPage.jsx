...
import { 
  ...
  CreditCard, Zap, Building2, ShieldAlert
} from 'lucide-react';
...
const RegisterPage = ({ onRegisterSuccess }) => {
  ...
  const handleNextStep = () => {
    if (step === 'account') {
      ...
      // Logic for Personal Plan: Skip payment if price is 0
  ...
  const completeRegistration = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Since isVerified is now true by default in backend, go straight to login
        alert("Registration Successful. Your identity has been authorized.");
        navigate('/login');
      } else {
  ...
  const getStepBranding = () => {
    switch(step) {
      ...
      case 'plan': return { tag: "Scaling Access", title: "Choose your level of protection.", desc: "Pick the tier that matches your security.", icon: <Zap size={40} className="text-[#123458]" /> };
      ...
      <div className="hidden md:flex flex-col justify-between w-[35%] lg:w-[40%] bg-[#123458] p-12 relative text-[#F1EFEC] shadow-2xl overflow-hidden">
        <div className="z-10 space-y-8">
          <div onClick={() => navigate('/')} className="flex items-center gap-3 cursor-pointer group w-fit">
            ...
            <span className="text-2xl font-black font-serif">Vault.io</span>
          </div>
          ...
          <div className="min-h-[380px]">
            {step === 'account' && (
              <div className="space-y-5 animate-content-smooth">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5"><label className={labelClass}>Full Name</label><div className="relative group"><User className={iconClass} size={18} /><input name="name" type="text" value={formData.name} onChange={handleInputChange} placeholder="Alex Morgan" className={inputClass} /></div></div>
                  <div className="space-y-1.5"><label className={labelClass}>Email</label><div className="relative group"><Mail className={iconClass} size={18} /><input name="email" type="email" value={formData.email} onChange={handleInputChange} placeholder="alex@vault.io" className={inputClass} /></div></div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-1.5"><label className={labelClass}>Password</label><div className="relative group"><Lock className={iconClass} size={18} /><input name="password" type="password" value={formData.password} onChange={handleInputChange} placeholder="••••••••" className={inputClass} /></div></div>
                  <div className="space-y-1.5"><label className={labelClass}>Confirm Password</label><div className="relative group"><ShieldAlert className={iconClass} size={18} /><input name="confirmPassword" type="password" value={formData.confirmPassword} onChange={handleInputChange} placeholder="••••••••" className={inputClass} /></div></div>
                </div>
              </div>
            )}
            ...
            {step === 'payment' && (
              <div className="space-y-6 animate-content-smooth">
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5"><label className={labelClass}>Card Information</label><div className="relative group"><CreditCard className={iconClass} size={18} /><input name="cardNumber" type="text" value={formData.cardNumber} onChange={handleInputChange} placeholder="0000 0000 0000 0000" className={inputClass} /></div></div>
                  <div className="grid grid-cols-2 gap-4">
                    <input name="expiry" type="text" value={formData.expiry} onChange={handleInputChange} placeholder="MM / YY" className={inputClass.replace('pl-12', 'pl-6')} />
                    <input name="cvc" type="text" value={formData.cvc} onChange={handleInputChange} placeholder="CVC" className={inputClass.replace('pl-12', 'pl-6')} />
                  </div>
                </div>
              </div>
            )}
